...
router.post('/', [auth,
    ...
    async (req, res) => {
        ...
        const {
            ...
            M_S_C,
            phone
        } = req.body; // destructuring the req body
        ...
        try {
            ...
            if (hospital) {
                hospital = await Hospitals.findOneAndUpdate({
                    user: req.user.id
                }, {
                    $set: hospitalProfile
                }, {
...
router.get('/user/:user_id', async (req, res) => {
    ...
    try {
        ...
        if (!hospital) return res.status(400).json({
            msg: "There is no hospital for this user"
        })
...
router.delete('/', auth, async (req, res) => {
    ...
    try {
        ...
        // remove profile 
        await Hospitals.findOneAndRemove({
            user: req.user.id
        })
