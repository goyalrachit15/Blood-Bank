...
router.put('/all/approve/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (user.email === 'Admin@admin.com') {
            ...
            if (post_owner) {
                post_owner.notifications.unshift({
                    msg: `your post ${post.text} has been approved by the admin `
                })
            ...
            return res.json({
                msg: 'Post ' + (req.body.Approvel === true ? 'Approved' : 'Not Approved')
            })
        }
...
router.put('/participants/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (post.participants.filter(part => part.user.toString() === req.user.id).length > 0) {
            return res.status(400).json({
                msg: 'Already joined as a participant'
            });
        }
        ...
        if (post_owner) {
            post_owner.notifications.unshift({
                msg: `${user.name} is joined in your post ${post.text}  as a participant`
            })
...
router.put('/rm-participants/:id', auth, async (req, res) => {
    ...
    try {
        ...
        if (post.participants.filter(part => part.user.toString() === req.user.id).length === 0) {
            return res.status(400).json({
                msg: 'not a participant for this post'
            });
        }
